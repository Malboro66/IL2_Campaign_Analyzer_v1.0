...
class SettingsTab(QWidget):
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        self._setup_ui(); self.load_settings()
    def _setup_ui(self) -> None:
        layout = QVBoxLayout(self); form = QFormLayout()
        self.cmb_theme = QComboBox(); self.cmb_theme.addItems(["Claro", "Escuro"])
        form.addRow(QLabel("Tema da interface:"), self.cmb_theme)
        self.cmb_export_format = QComboBox(); self.cmb_export_format.addItems(["TXT", "PDF", "CSV"])
        form.addRow(QLabel("Formato padrão de exportação:"), self.cmb_export_format)
        self.btn_export_folder = QPushButton("Selecionar pasta…"); self.lbl_export_folder = QLabel("Nenhuma pasta definida")
    ...
    def load_settings(self) -> None:
        theme = self.settings.value("theme", "Claro"); export_format = self.settings.value("export_format", "TXT")
